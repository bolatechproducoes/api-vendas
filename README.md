...
2. Renomear o arquivo ```.env.example.docker-compose``` para ```.env``` (Ele já esta configurado, é só alterar o nome que a aplicação irá funcionar).
...
8. Você pode rodar os testes do Jest executando na pasta do projeto o comando: ```npm run test```, (o docker-compose up deve estar rodando) depois você pode rodar ```npm run test:coverage``` e acessar o relatório de cobertura dos testes abrindo o arquivo ```/coverage/lcov-report/index.html``` no seu navegador, para rodar os teste em watch mode você pode utilizar o script: ```npm run test:silent``` e utilizar a opção ```o``` para ficar executando os testes somente nos arquivos que sofrerem alteração e não foram commitados(caso receba mensagem de 'access error' na hora de executar os testes execute ```npm i``` no terminal na pasta do projeto que resolverá).
...
3. Executar as [migrações das tabelas](https://github.com/bolatechproducoes/api-vendas#comandos-cli) com o comando ```npm run -- -d src/shared/infra/typeorm/index.ts typeorm migration:run``` executado no terminal na pasta do projeto.
...
7. Executar o comando: ```npm run dev```
...
```npm run typeorm migration:create -n NOMEDAMIGRAÇÂO```

* Comando para criar uma migração com o TypeORM no TypeORM 3.x:
```npm run typeorm migration:create ENDERECODAMIGRACAO NOMEDATABELA```

* Comando para executar migração no TypeORM no TypeORM 2.x:
```npm run typeorm migration:run```

* Comando para executar migração no TypeORM no TypeORM 3.x:
```npm run -- -d ENDERECODOARQUIVODECONFIGDASMIGRACOES typeorm migration:run```
